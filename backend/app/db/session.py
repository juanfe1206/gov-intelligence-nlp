...
# Pooler-friendly mode: disable prepared statement caching to work with pgbouncer
# and avoid issues with connection poolers (Supabase, etc.)
engine_kwargs: dict = {
    "echo": settings.APP_ENV == "dev",  # Log SQL queries only in dev environment
    "future": True,
    "connect_args": {
        # Required for pgbouncer compatibility (transaction/statement pool modes)
